...
int main(){
	...
	//EXAMEN: EJERCICIO 3
	float temperaturas[8]={23,12.5, 201,24,15,20,-25.2};
	float temp_a;

	for(int i = 0; i<8; i++){
	if(temperaturas[i] > MAX_TEMP || temperaturas[i]< MIN_TEMP){
	   printf("Temperatura atípica: %d \n", i);
	i = temp_a;
	   printf("La temperatura debe ser considerada o eliminada: %f \n", temp_a);
	}
